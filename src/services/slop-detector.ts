...
const RU_SLOP_RULES: SlopRule[] = [
  ...
  // ── HIGH severity: variants PW2 missed (pw3/step2) ──────────────────────
  {
    name: 'не_X_именно_Y_split',
    test: () => /это не \w+(\s+\w+)?\.?\s+(И?менно|Это)\s+\w+/iu,
    severity: 'high',
    fix: 'Вариация "не X, а Y" через точку. Замени конкретной сценой.',
  },
  {
    name: 'не_просто_X',
    test: () => /не\s+просто\s+\w+[,.]?\s+(\w+\s+){0,3}(а|именно|это)/iu,
    severity: 'high',
    fix: '"Не просто X, а Y" — слоп. Замени конкретикой.',
  },
  {
    name: 'X_это_результат_Y',
    test: () => /\w+\s+—\s+это\s+(не\s+)?(результат|залог|секрет|основа|следствие)\s+\w+/iu,
    severity: 'high',
    fix: '"X — это результат/залог Y" — абстракция. Замени на конкретный факт.',
  },
  {
    name: 'не_а_настоящий',
    test: () => /не \w+(\s+с\s+\w+)?,\s*а\s+(настоящ|истинн|реальн)\w*\s+\w+/iu,
    severity: 'high',
    fix: '"Не X, а настоящий Y" — слоп. Конкретика вместо оценки.',
  },
  {
    name: 'abstract_chain',
    test: () => /(результат|залог|секрет|следствие)\s+(правильн|качествен|профессионал)\w+\s+(техник|подход|работ|материал)\w*/iu,
    severity: 'high',
    fix: 'Абстрактная цепочка. Замени конкретным действием/брендом/числом.',
  },
  {
    name: 'гарантирует_абстракцию',
    test: () => /(гарантирует|обеспечивает|дарит|создаёт)\s+(стойкост|комфорт|красот|элегантност|роскош)/iu,
    severity: 'high',
    fix: '"Гарантирует стойкость" — шаблон. Покажи стойкость через конкретный пример.',
  },
  {
    name: 'generic_cta_подарите',
    test: () => /подарите?\s+себе/iu,
    severity: 'high',
    fix: '"Подарите себе" — эмоциональный шантаж. Конкретный ask вместо.',
  },

  // ── MEDIUM severity: patterns from PW3 research (pw3/step2) ────────────
  {
    name: 'плотный_график',
    test: () => /(плотн|насыщен|интенсивн)\w+\s+график/iu,
    severity: 'medium',
    fix: '"Плотный график" — клише. Назови конкретную ситуацию.',
  },
  {
    name: 'поможет_почувствовать',
    test: () => /(поможет|позволит)\s+(почувствовать|ощутить)\s+(себя|разниц)/iu,
    severity: 'medium',
    fix: 'Абстрактное обещание. Опиши конкретный момент.',
  },
  {
    name: 'детали_которые',
    test: () => /детал\w+,?\s+(которые|что)\s+(не\s+видны|остаются|часто)/iu,
    severity: 'medium',
    fix: '"Детали которые не видны" — шаблон. Назови конкретную деталь.',
  },
  {
    name: 'generic_hook_знаете_почему',
    test: () => /^знаете?,?\s*(почему|что|как)/iu,
    severity: 'medium',
    fix: '"Знаете почему" — банальный заход. Начни с факта или сцены.',
  },
  {
    name: 'generic_hook_все_мечтают',
    test: () => /^все\s+мечтают/iu,
    severity: 'medium',
    fix: 'Банальный hook. Начни конкретно.',
  },
  {
    name: 'generic_hook_хотите',
    test: () => /^хотите\s+(чтобы|узнать|маникюр)/iu,
    severity: 'medium',
    fix: '"Хотите..." — слабый hook. Начни с утверждения или сцены.',
  },
  {
    name: 'эксклюзивный_сервис',
    test: () => /эксклюзивн\w+\s+(серви|подход|обслужива)/iu,
    severity: 'medium',
    fix: '"Эксклюзивный сервис" — маркетинговый штамп. Удали.',
  },
  {
    name: 'ваше_преображение_v2',
    test: () => /ваше?\s+(стиль|ритм|индивидуальност|преображени)/iu,
    severity: 'medium',
